...
function update(data) {
    var grade = this.find(data); grade.count++;
    return grade;
}
...
function genArray(length) {
    ...
    return arr; }
load("BST"); // 记得将 update() 方法加进 BST 类定义
var grades = genArray(100);
prArray(grades);
var gradedistro = new BST();
for (var i = 0; i < grades.length; ++i) {
    var g = grades[i];
    var grade = gradedistro.find(g); if (grade == null) {
...
var cont = "y";
while (cont == "y") { putstr("\n\nEnter a grade: "); var g = parseInt(readline());
    var aGrade = gradedistro.find(g); if (aGrade == null) {
